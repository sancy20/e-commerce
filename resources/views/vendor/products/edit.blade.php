            ...
            <h2 class="text-xl font-semibold mb-4">Base Product Information</h2>
             <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" class="shadow w-full" value="{{ old('name', $product->name) }}" required>
                ...
                <div>
                    <label class="block text-sm font-bold mb-2">Base Price:</label>
                    <input type="number" name="price" step="0.01" class="shadow w-full" value="{{ old('price', $product->price) }}" required>
                    @error('price') <p class="text-red-500 text-xs italic">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold mb-2">Base SKU:</label>
                    <input type="text" name="sku" class="shadow w-full" value="{{ old('sku', $product->sku) }}">
                    @error('sku') <p class="text-red-500 text-xs italic">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm font-bold mb-2">Base Stock:</label>
                    <input type="number" name="stock_quantity" class="shadow w-full" value="{{ old('stock_quantity', $product->stock_quantity) }}" required>
                    @error('stock_quantity') <p class="text-red-500 text-xs italic">{{ $message }}</p> @enderror
                </div>
                <div class="flex items-center">
                    <input type="checkbox" name="is_featured" value="1" class="mr-2" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }}>
                    <label class="text-sm font-bold">Is Featured?</label>
                </div>
            </div>
