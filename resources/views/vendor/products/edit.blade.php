        ...
        <form action="{{ route('vendor.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="shadow w-full p-2 border rounded" value="{{ old('name', $product->name) }}" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" step="0.01" class="shadow w-full p-2 border rounded" value="{{ old('price', $product->price) }}" required>
                </div>
                <div>
                    <label for="sku" class="block text-sm font-bold mb-2">Base SKU:</label>
                    <input type="text" name="sku" id="sku" class="shadow w-full p-2 border rounded" value="{{ old('sku', $product->sku) }}">
                </div>
                <div>
                    <label for="stock_quantity" class="block text-sm font-bold mb-2">Base Stock:</label>
                    <input type="number" name="stock_quantity" id="stock_quantity" class="shadow w-full p-2 border rounded" value="{{ old('stock_quantity', $product->stock_quantity) }}" required>
                </div>
                <div class="flex items-center self-end">
                    <input type="checkbox" name="is_featured" value="1" id="is_featured" class="mr-2" {{ old('is_featured', $product->is_featured) ? 'checked' : '' }}>
            ...
            <div id="variants-section" class="border-t pt-4 mt-4">
                ...
                <button type="button" id="add-variant-btn" class="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded mt-4" disabled>Add New Variant</button>
            </div>
            
            <div class="flex justify-end mt-6 border-t pt-6">
                <button type="submit" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">Update Product</button>
            </div>
        </form>
    </div>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchAttributesForCategory(categoryId) {
        ...
        try {
            ...
            variantsContainer.innerHTML = '<p class="text-red-500 font-bold">Could not load attributes.</p>';
        }
    }
    ...
    function createVariantRowHtml(isNew, data) {
        ...
        const imagePreviewHtml = data.image ? `<div class="mt-2"><p class="text-xs">Current: <img src="{{ asset('storage') }}/${data.image}" class="h-10 w-10 object-cover"></p><label class="text-sm"><input type="checkbox" name="${namePrefix}[remove_image]" value="1"> Remove</label></div>` : '';
        ...
        <div class="variant-row border p-4 rounded-lg mb-4 bg-gray-50" data-variant-id="${data.id || 'new'}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="col-span-full">
                    <label class="block text-sm font-bold mb-2">Variant Image (Optional):</label>
