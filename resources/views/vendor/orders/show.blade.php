...
@section('content')
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('vendor.orders.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Back to My Orders</a>
    </div>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="mb-2"><strong class="font-medium">Total Order Amount:</strong> <span class="font-bold text-lg text-blue-700">${{ number_format($order->total_amount, 2) }}</span></p>
            ...
            <div>
                <h2 class="text-xl font-semibold text-gray-800 mb-3">Customer Addresses</h2>
                <div class="mb-4">
        ...
        {{-- Order Items (Filtered for this vendor's products) --}}
        <div class="mt-8 border-t border-gray-200 pt-6">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Ordered Items (My Products)</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Product</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price (at order)</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center text-gray-500">No products from your store in this order.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="mt-8 flex justify-end space-x-4">
            <a href="{{ route('vendor.orders.edit', $order->id) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">Edit Order Status</a>
            <form action="{{ route('vendor.orders.destroy', $order->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete order {{ $order->order_number }}? This action is irreversible and affects reporting.');">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Delete Order</button>
            </form>
        </div>
    </div>
@endsection