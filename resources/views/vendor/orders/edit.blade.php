            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <ul class="list-disc list-inside mb-4">
                        @php
                            $vendorItems = $order->orderItems->filter(fn($item) => optional($item->product)->vendor_id === Auth::id());
                        @endphp
                        @forelse($vendorItems as $item)
                            <li>{{ $item->product->name ?? 'Product N/A' }} ({{ optional($item->productVariant)->variant_name ?? 'Base Product' }}) x {{ $item->quantity }} @ ${{ number_format($item->price, 2) }}</li>
                        @empty
                ...
                <div>
                    <h2 class="text-xl font-semibold text-gray-800 mb-3">Update Your Items</h2>
                    <div class="mb-4">
                        <label for="order_status" class="block text-gray-700 text-sm font-bold mb-2">Order Status:</label>
                        <select name="order_status" id="order_status" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" required>
                        ...
                        @error('order_status')<p class="text-red-500 text-xs italic">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="notes" id="notes" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">{{ old('notes', $order->notes) }}</textarea>
