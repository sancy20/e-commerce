...
<div class="flex justify-between items-center mb-4">
    ...
    <a href="{{ route('admin.orders.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Back to Orders</a>
</div>
...
<div class="bg-white shadow-md rounded-lg p-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <p class="mb-2"><strong class="font-medium">Customer:</strong> {{ $order->user->name ?? 'Guest User' }} ({{ $order->user->email ?? 'N/A' }})</p>
    ...
    <div class="mt-8 flex justify-end space-x-4">
        <a href="{{ route('admin.orders.edit', $order->id) }}" class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">Edit Order Status</a>
        <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete order {{ $order->order_number }}? This action is irreversible and affects reporting.');">
            @csrf
            @method('DELETE')
